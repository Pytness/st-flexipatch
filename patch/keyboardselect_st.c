void set_notifmode(int type, KeySym ksym)
{
    ...
    if (type < 2) {
        ...
        for (deb = &term.line[bot][col - 6], fin = &term.line[bot][col]; deb < fin; z++, deb++)
            deb->mode = ATTR_REVERSE, deb->u = *z, deb->fg = defaultfg, deb->bg = defaultbg;
    } else if (type < 5)
        memcpy(term.line[bot], g, col * sizeof(Glyph));
    else {
        for (deb = &term.line[bot][0], fin = &term.line[bot][col]; deb < fin; deb++)
            deb->mode = ATTR_REVERSE, deb->u = ' ', deb->fg = defaultfg, deb->bg = defaultbg;
...
void select_or_drawcursor(int selectsearch_mode, int type)
{
    ...
    if (selectsearch_mode & 1) {
        ...
        xdrawcursor(term.c.x,
                    term.c.y,
                    getglyph(term, term.c.y, term.c.x),
                    term.ocx,
                    term.ocy,
                    getglyph(term, term.ocy, term.ocx));
#else
        xdrawcursor(
            term.c.x, term.c.y, term.line[term.c.y][term.c.x], term.ocx, term.ocy, term.line[term.ocy][term.ocx]);
