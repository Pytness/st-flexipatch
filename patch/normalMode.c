...
ExitState kPressHist(const char* cs, size_t len, int ctrl, const KeySym* kSym) {
	...
	if (result != failed) {
		...
		if ((state.cmd.op == visualLine) || ((state.cmd.op == yank) && (result == exitOp))) {
			const int off =
			    term.c.y + (IS_SET(MODE_ALTSCREEN) ? 0 : histOff) < sel.ob.y; //< Selection start below end.
	...
	styleCmd =
	    style[state.cmd.op == yank ? 1 : (state.cmd.op == visual ? 2 : (state.cmd.op == visualLine ? 3 : 0))];
