...
extern const Glyph styleSearch, style[];
extern const char wDelS[], wDelL[], *nmKeys[];
...
static inline int contains(Rune l, const char* values, const size_t memSize)
{
...
static inline void decodeTo(const char* cs, size_t len, DynamicArray* arr)
{
    char* var = expand(arr);
    if (!var)
        empty(arr);
...
{ //    ({ =>)       l  ?  {  \n | l | v  /  } \n | h | y
    ...
    if (lc == 'w')
        mot[2] = (char)('b' - lc + l), mot[7] = (char)((a ? 'w' : 'e') - lc + l), mot[9] = (char)(a ? 'h' : 0);
    else {
...
ExitState kPressHist(const char* cs, size_t len, int ctrl, const KeySym* kSym)
{
    historyOpToggle(1, 1);
    const int prevYOff = IS_SET(MODE_ALTSCREEN) ? 0 : histOff, search = state.m.search && state.m.active,
              prevAltToggle = altToggle, prevOverlay = overlay;
    const int noOp = !state.cmd.op && !state.cmd.infix, num = len == 1 && BETWEEN(cs[0], 48, 57),
    ...
    if (esc || ret || ins) {
        ...
        state.m.searchPos = (Pos){
            .p = {term.c.x, term.c.y, prevYOff}
        };
        state.m.active = 1;
        ...
        for (size_t i = 0; !ctrl && i < amountNmKeys; ++i)
            if (cs[0] == nmKeys[i][0] && failed != (result = pressKeys(&nmKeys[i][1], strlen(nmKeys[i]) - 1)))
                goto end;
    ...
    if (result != failed) {
        ...
        if ((state.cmd.op == visualLine) || ((state.cmd.op == yank) && (result == exitOp))) {
            const int off = term.c.y + (IS_SET(MODE_ALTSCREEN) ? 0 : histOff) < sel.ob.y; //< Selection start below end.
    ...
    styleCmd = style[state.cmd.op == yank ? 1 : (state.cmd.op == visual ? 2 : (state.cmd.op == visualLine ? 3 : 0))];
    const int posLin = !IS_SET(MODE_ALTSCREEN) ? rangeY(insertOff - histOff) : 0, h = rows() - term.row;
...
void historyOverlay(int x, int y, Glyph* g)
{
    if (!histMode)
        return;
    const TCursor* cHist = histOp ? &term.c : &c[0];
