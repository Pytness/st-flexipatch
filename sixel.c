...
static const sixel_color_t sixel_default_color_table[] = {
...
static int
sixel_image_init(sixel_image_t* image, int width, int height, int fgcolor, int bgcolor, int use_private_register)
{
    int status = (-1);
    size_t size;
...
static int image_buffer_resize(sixel_image_t* image, int width, int height)
{
    ...
    if (width > image->width) { /* if width is extended */
        for (n = 0; n < min_height; ++n) {
            ...
            memset(alt_buffer + width * n + image->width, 0, (size_t)(width - image->width) * sizeof(sixel_color_no_t));
        }
    } else {
        for (n = 0; n < min_height; ++n) {
            /* copy from source image */
            memcpy(alt_buffer + width * n, image->data + image->width * n, (size_t)width * sizeof(sixel_color_no_t));
        }
    }
