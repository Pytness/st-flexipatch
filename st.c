...
int historyBufferScroll(int n) {
	if (IS_SET(MODE_ALTSCREEN) || !n)
		return histOp;
	int p = abs(n = (n < 0) ? max(n, -term.row) : min(n, term.row)), r = term.row - p, s = sizeof(*term.dirty),
	...
	const int prevOffBuf = sel.alt ? 0 : insertOff + term.row;
	if (sel.ob.x != -1 && !histOp && n) {
		const int offBuf = sel.alt ? 0 : insertOff + term.row, pb = rangeY(sel.ob.y - prevOffBuf),
			  pe = rangeY(sel.oe.y - prevOffBuf);
		const int b = rangeY(sel.ob.y - offBuf), nln = n < 0, e = rangeY(sel.oe.y - offBuf),
			  last = offBuf - nln;
		if (pb != b && ((pb < b) != nln))
			sel.ob.y = last;
		if (pe != e && ((pe < e) != nln))
...
void selnormalize(void) {
	...
	const int oldb = sel.nb.y, olde = sel.ne.y;
	if (sel.ob.x == -1) {
		sel.ne.y = sel.nb.y = -1;
	} else {
		const int offsetBuffer = sel.alt ? 0 : insertOff + term.row;
		const int off	       = sel.alt ? 0 : (histMode ? histOff : insertOff);
		const int nby = rangeY(sel.ob.y - off), ney = rangeY(sel.oe.y - off);
		sel.swap      = rangeY(sel.ob.y - offsetBuffer) > rangeY(sel.oe.y - offsetBuffer);
		sel.nb.y      = sel.swap ? ney : nby;
		sel.ne.y      = !sel.swap ? ney : nby;
		const int cnb = sel.nb.y < term.row, cne = sel.ne.y < term.row;
	...
	const int nBet = sel.nb.y <= sel.ne.y, oBet = oldb <= olde;
	for (int i = 0; i < term.row; ++i) {
		const int n = nBet ? BETWEEN(i, sel.nb.y, sel.ne.y) : OUT(i, sel.nb.y, sel.ne.y);
		term.dirty[i] |=
		    (sel.type == SEL_RECTANGULAR && n) || (n != (oBet ? BETWEEN(i, oldb, olde) : OUT(i, oldb, olde)));
	}
...
#if VIM_BROWSE_PATCH
	return ((sel.nb.y > sel.ne.y) ? OUT(y, sel.nb.y, sel.ne.y) : BETWEEN(y, sel.nb.y, sel.ne.y))
...
#if VIM_BROWSE_PATCH
	const int start = sel.swap ? sel.oe.y : sel.ob.y, h = rows();
...
void tclearregion(int x1, int y1, int x2, int y2) {
	...
	for (y = y1; y <= y2; y++) {
		...
		for (x = x1; x <= x2; x++) {
			gp = &term.line[y][x];
#if !NON_SELECTION_CLEAR_PATCH
			if (selected(x, y))
				selclear();
#endif // !NON_SELECTION_CLEAR_PATCH
...
void csihandle(void) {
		...
		case 2: /* screen */
			...
			if (!IS_SET(MODE_ALTSCREEN)) {
				...
				for (n = term.row - 1; n >= 0; n--) {
					for (m = 0; m < maxcol && term.line[n][m].u == ' ' && !term.line[n][m].mode;
					     m++)
						;
					if (m < maxcol) {
#if SCROLLBACK_PATCH
	...
	case 'n': /* DSR – Device Status Report (cursor position) */
		if (csiescseq.arg[0] == 6) {
			len = snprintf(buffer, sizeof(buffer), "\033[%i;%iR", term.c.y + 1, term.c.x + 1);
...
void osc4_color_response(int num) {
	...
	n = snprintf(buf, sizeof buf, "\033]4;%d;rgb:%02x%02x/%02x%02x/%02x%02x\007", num, r, r, g, g, b, b);
...
void osc_color_response(int index, int num) {
	...
	n = snprintf(buf, sizeof buf, "\033]%d;rgb:%02x%02x/%02x%02x/%02x%02x\007", num, r, r, g, g, b, b);
	...
	case 'P': /* DCS -- Device Control String */
		...
		if (IS_SET(MODE_SIXEL)) {
			...
			for (i = 0; i < (sixel_st.image.height + win.ch - 1) / win.ch; ++i) {
				int x;
				tclearregion(term.c.x,
					     term.c.y,
					     term.c.x + (sixel_st.image.width + win.cw - 1) / win.cw,
					     term.c.y);
				for (x = term.c.x;
				     x < MIN(term.col, term.c.x + (sixel_st.image.width + win.cw - 1) / win.cw);
				     x++)
...
#endif // SCROLLBACK_PATCH
#if VIM_BROWSE_PATCH
	const int colSet = col, alt = IS_SET(MODE_ALTSCREEN), ini = buf == NULL;
	col		 = MAX(col, buffCols);
	row		 = MIN(row, buffSize);
	const int minrow = MIN(row, term.row), mincol = MIN(col, buffCols);
#elif COLUMNS_PATCH
	int tmp = col;
	int minrow, mincol;
...
void drawregion(int x1, int y1, int x2, int y2) {
	...
	if (altToggle && histMode && !histOp)
		memset(term.dirty, 0, sizeof(*term.dirty) * term.row);
	const int o = !IS_SET(MODE_ALTSCREEN) && histMode && !histOp, h = rows();
#endif // VIM_BROWSE_PATCH
	int y;

	for (y = y1; y < y2; y++) {
#if VIM_BROWSE_PATCH
		const int oy = o ? (y + insertOff - histOff + h) % h : y;
