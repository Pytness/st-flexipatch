...
int historyBufferScroll(int n)
{
    ...
    if (sel.ob.x != -1 && !histOp && n) {
        ...
        const int b = rangeY(sel.ob.y - offBuf), nln = n < 0, e = rangeY(sel.oe.y - offBuf), last = offBuf - nln;
        if (pb != b && ((pb < b) != nln))
            sel.ob.y = last;
        if (pe != e && ((pe < e) != nln))
            sel.oe.y = last;
            ...
            if (!IS_SET(MODE_ALTSCREEN)) {
                ...
                for (int i = 0; i < buffSize; ++i) {
                    if (!BETWEEN(i, insertOff, insertOff + term.row - 1)
                        && !(insertOff + term.row > buffSize && BETWEEN(i, 0, (insertOff + term.row - 1) % buffSize))) {
