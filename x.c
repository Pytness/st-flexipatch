...
int mouseaction(XEvent* e, uint release)
{
    ...
    for (ms = mshortcuts; ms < mshortcuts + LEN(mshortcuts); ms++) {
        if (ms->release == release && ms->button == e->xbutton.button && (!ms->screen || (ms->screen == screen))
...
#if VIM_BROWSE_PATCH
        const int tripleClick = TIMEDIFF(now, xsel.tclick2) <= tripleclicktimeout,
...
void propnotify(XEvent* e)
{
    ...
    if (pseudotransparency && !strncmp(XGetAtomName(xw.dpy, e->xproperty.atom), "_NET_WM_STATE", 13)) {
        updatexy();
        redraw();
    }
...
void selrequest(XEvent* e)
{
    ...
    if (xsre->target == xa_targets) {
        ...
        string = xsel.xtarget;
        XChangeProperty(
            xsre->display, xsre->requestor, xsre->property, XA_ATOM, 32, PropModeReplace, (uchar*)&string, 1);
...
{
    const float usedAlpha = focused ? alpha : alphaUnfocused;
    if (opt_alpha)
...
/* white cursor, black outline */
    ...
    XChangeProperty(xw.dpy, xw.win, xw.netwmicon, XA_CARDINAL, 32, PropModeReplace, (uchar*)&icon, LEN(icon));
    ...
    XChangeProperty(xw.dpy, xw.win, motifwmhints, motifwmhints, 16, PropModeReplace, (unsigned char*)data, 5);
...
/* Intelligent cleaning up of the borders. */
    ...
    if (winx + width >= borderpx + win.tw) {
        xclear(
            winx + width, (y == 0) ? 0 : winy, win.w, ((winy + win.ch >= borderpx + win.th) ? win.h : (winy + win.ch)));
    }
    if (y == 0)
...
/* Clean up the region we want to draw to. */
    ...
    XftDrawGlyphFontSpec(xw.draw, fg, specs, len);
        ...
        if (base.mode & ATTR_UNDERLINE) {
            ...
            XGCValues ugcv = {
                .foreground = linecolor, .line_width = wlw, .line_style = LineSolid, .cap_style = CapNotLast
            };

            GC ugc = XCreateGC(
                xw.dpy, XftDrawDrawable(xw.draw), GCForeground | GCLineWidth | GCLineStyle | GCCapStyle, &ugcv);

            // Underline Style
            if (base.ustyle != 3) {
                XFillRectangle(
                    xw.dpy, XftDrawDrawable(xw.draw), ugc, winx, winy + dc.font.ascent * chscale + 1, width, wlw);
                ...
                */
                ...
                if (npoints >= 3) {
                    ...
                    // Last point
                    points[npoints - 1] =
                        (XPoint){ .x = ww / 2, .y = (isSlopeRising(wx, npoints - 1, ww)) ? wh / 2 : -wh / 2 };
                    waveLength += ww / 2;
...
void xfinishdraw(void)
{
    ...
    for (im = term.images; im; im = im->next) {
        ...
        if (!im->pixmap) {
            im->pixmap = (void*)XCreatePixmap(xw.dpy,
                                              xw.win,
                                              im->width,
                                              im->height,
#if ALPHA_PATCH
        ...
        XCopyArea(xw.dpy,
                  ...
                  im->width,
                  im->height,
                  borderpx + im->x * win.cw,
                  borderpx + im->y * win.ch);
#endif // ANYSIZE_PATCH
        XFreeGC(xw.dpy, gc);
    }
#endif // SIXEL_PATCH
...
void kpress(XEvent* ev)
{
    ...
    for (bp = shortcuts; bp < shortcuts + LEN(shortcuts); bp++) {
        if (ksym == bp->keysym && match(bp->mod, e->state) && (!bp->screen || bp->screen == screen)) {
            bp->func(&(bp->arg));
            return;
        }
    }
...
#if BACKGROUND_IMAGE_PATCH
    if (pseudotransparency) {
        if (e->xconfigure.width == win.w && e->xconfigure.height == win.h && e->xconfigure.x == win.x
            && e->xconfigure.y == win.y)
            return;
        updatexy();
    } else
#endif // BACKGROUND_IMAGE_PATCH
